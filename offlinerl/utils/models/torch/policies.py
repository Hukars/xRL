...
class SquashedNormalPolicy(Policy):
    ...
    def act_log_prob(self, obs, act, with_entropy = False) -> torch.Tensor:
        act = torch.clip(act, -1 + 1e-6, 1 - 1e-6)
        dist = self.dist(obs)
        sample_act = torch.tanh(dist.rsample())
        sample_act = torch.clip(sample_act, -1 + 1e-6, 1 - 1e-6)
        
        if with_entropy:
            return squash_action(dist, torch.atanh(act))[1], - squash_action(dist, torch.atanh(sample_act))[1]
        else:
            return squash_action(dist, torch.atanh(act))[1]
