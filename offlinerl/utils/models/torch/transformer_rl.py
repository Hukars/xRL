...
class PositionalEmbedding(torch.nn.Module):
    def __init__(self, dim, device):
        ...
        self.dim = dim
        inv_freq = 1 / (10000 ** (torch.arange(0.0, dim, 2.0) / dim)).to(device)
...
class StableTransformerXL(torch.nn.Module):
    def __init__(
        ...
        (
            ...
            self.encoder_size,
            self.device,
        ) = (n_layers, n_heads, d_input, d_head_inner, d_ff_inner, transition_num, encoder_size, device)

        self.pos_embs = PositionalEmbedding(d_input, device)
        ...
        self.fc1 = nn.Linear(self.transition_num * self.d_input, encoder_size)

    def init_memory(self, shape):
        return [
            # torch.empty(0, dtype=torch.float).to(device)
            torch.zeros(size=shape, dtype=torch.float).to(self.device)
    ...
    def forward(self, inputs, memory=None):
        ...
        if memory is None:
            memory = self.init_memory(inputs.shape)
...
if __name__ == "__main__":
    model = StableTransformerXL(d_input=83, n_layers=4, n_heads=3, d_head_inner=64, d_ff_inner=128, 
        transition_num=20, encoder_size=32, device=torch.device('cpu'))
    input = torch.randn(100, 20, 83)
    model.get_encoder_vector(input)
